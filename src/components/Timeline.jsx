import { filterEvents, periodLabel } from "../lib/basketball.js";
  ...
  return (
    <div className="modal-backdrop information-backdrop" onMouseDown={onClose}>
      <section
        ...
        style={{
          "--detail-color": tag?.color || team?.primaryColor || event.color,
        }}
      >
        <div className="information-popup-hero">
          ...
          <div>
            ...
            <h2>{title}</h2>
            <p>
              Acción registrada en{" "}
              {formatTime(event.anchor ?? event.start, true)}
            </p>
          </div>
        ...
        <div className="information-facts">
          {type === "team" && (
            <>
              <div>
                <span>Ciudad</span>
                <strong>{team?.city || "Sin indicar"}</strong>
              </div>
              <div>
                <span>Categoría</span>
                <strong>{team?.category || "Sin indicar"}</strong>
              </div>
              <div>
                <span>Pabellón</span>
                <strong>{team?.arena || "Sin indicar"}</strong>
              </div>
              <div>
                <span>Plantilla</span>
                <strong>{team?.players?.length || 0} jugadores</strong>
              </div>
            </>
          )}
          {type === "player" && (
            <>
              ...
              <div>
                <span>Posición</span>
                <strong>{player?.position || "Sin indicar"}</strong>
              </div>
              <div>
                <span>Altura</span>
                <strong>
                  {player?.height ? `${player.height} cm` : "Sin indicar"}
                </strong>
              </div>
              <div>
                <span>Equipo</span>
                <strong>{team?.name || event.team || "Sin indicar"}</strong>
              </div>
            </>
          )}
          {type === "tag" && (
            <>
              ...
              <div>
                <span>Zona</span>
                <strong>
                  {zone?.name || event.shotZoneName || "Sin registrar"}
                </strong>
              </div>
              <div>
                <span>Valor</span>
  ...
  return (
    <section className="timeline-panel">
      <div className="section-heading">
        <div>
          <span className="eyebrow">Registro del partido</span>
        ...
        <div className="timeline-search">
          ...
          <span className="counter">{orderedEvents.length} acciones</span>
        </div>
      </div>
      ...
      <div
        ...
        onPointerMove={(event) => {
          if (draggingRef.current && (event.buttons & 1) === 1)
            seekFromPointer(event);
        }}
        onPointerUp={() => {
      ...
      >
        ...
        {events.map((event) => {
          ...
          const width = Math.max(
            ((event.end - event.start) / safeDuration) * 100,
            0.35,
          );
          return (
            <button
                  ...
                  <input
                    ...
                    onChange={() => {
                      ...
                      events.forEach((event) => {
                        if (selectAll !== selectedIds.has(event.id))
            ...
            <tbody>
              {orderedEvents
                .slice(
                  Math.min(
                    page,
                    Math.max(0, Math.ceil(orderedEvents.length / 50) - 1),
                  ) * 50,
                  (Math.min(
                    page,
                    Math.max(0, Math.ceil(orderedEvents.length / 50) - 1),
                ...
                .map((event) => (
                  <tr
                    key={event.id}
                    className={selectedIds.has(event.id) ? "selected" : ""}
                  >
                    <td>
                      <input
                        type="checkbox"
                    ...
                    <td>
                      {event.team ? (
                        <button
                          ...
                          style={{
                            "--event-team-color":
                              teamsById.get(event.teamId)?.primaryColor ||
                              "#64748b",
                          }}
                    ...
                    <td>
                      <div className="row-actions">
                        <button
                          ...
                          aria-label={`Destacar ${event.tagName}`}
                          aria-pressed={Boolean(event.favorite)}
      ...
      {orderedEvents.length > 50 && (
        <div className="pagination">
          ...
          <span>
            {Math.min(page + 1, Math.ceil(orderedEvents.length / 50))} /{" "}
            {Math.ceil(orderedEvents.length / 50)}
          </span>
          <button
            className="button ghost"
            disabled={(page + 1) * 50 >= orderedEvents.length}
            onClick={() => setPage((p) => p + 1)}
          >
            Siguiente
          </button>
