...
import { isShotTag, zoneStats } from "../lib/shotZones.js";
import { ShotCourtHeatmap } from "./ShotCourt.jsx";

function teamForEvent(project, event) {
  return project.teams.find(
    (team) =>
      team.id === event.teamId || (!event.teamId && team.name === event.team),
  );
}
...
function TrendChart({ events, duration, color }) {
  ...
  const safeDuration = Math.max(
    Math.max(
      duration || 0,
      events.reduce((max, event) => Math.max(max, event.end || 0), 1),
    ),
    1,
  );
  events.forEach((event) => {
    const bucket = Math.min(
      buckets.length - 1,
      Math.floor(
        ((event.anchor ?? event.start) / safeDuration) * buckets.length,
      ),
    );
  ...
  return (
    <div className="bi-trend-chart">
      <svg
        viewBox="0 0 800 220"
        role="img"
        aria-label="Evolución temporal de acciones"
      >
        <defs>
      ...
      <div className="trend-axis">
        <span>Inicio</span>
        <span>{formatTime(safeDuration * 0.25)}</span>
        <span>{formatTime(safeDuration * 0.5)}</span>
        <span>{formatTime(safeDuration * 0.75)}</span>
        <span>Final</span>
      </div>
    </div>
  );
  ...
  return (
    <aside className="bi-filter-rail">
      ...
      <div className="bi-data-status">
        <i />
        <div>
          <strong>Modelo actualizado</strong>
          <span>{project.events.length} registros disponibles</span>
        </div>
      </div>
    </aside>
  );
...
function MatchStatsPanel({ project }) {
  ...
  const filteredEvents = useMemo(
    ...
    [playerId, project.events, scope, tagId, teamId, period],
  );
  const stats = statisticsFor(project.template.tags, filteredEvents);
  const chartColor = selectedTeam?.primaryColor || "var(--teal)";
  const intervalCount = filteredEvents.filter(
    (event) => event.mode === "interval",
  ).length;
  const activePlayers = new Set(
    filteredEvents.map((event) => event.playerId).filter(Boolean),
  ).size;
  const shotRows = zoneStats(filteredEvents);
  const box = boxScore(filteredEvents);
  const shots = box.fga,
    madeShots = box.fgm;
  const shotPercentage = box.fg === null ? "—" : box.fg.toFixed(1);
  const taggedDuration = coveredSeconds(filteredEvents);
  ...
  const completenessRows = [
    {
      ...
      value: dataPercentage(
        filteredEvents.filter((event) => event.teamId).length,
      ),
    ...
    },
    {
      ...
      value: dataPercentage(
        filteredEvents.filter((event) => event.playerId).length,
      ),
    ...
    },
    {
      ...
      value: dataPercentage(
        filteredEvents.filter((event) => event.shotZoneId && isShotTag(event))
          .length,
        shots,
      ),
    ...
    },
    {
      ...
      value: dataPercentage(
        filteredEvents.filter((event) => event.notes?.trim()).length,
      ),
  ...
  const safeDuration = Math.max(
    project.video?.duration ||
      filteredEvents.reduce((max, event) => Math.max(max, event.end || 0), 0),
    1,
  );
  const periodRows = [...periods, ""].map((period) => {
    const events = filteredEvents.filter((e) => (e.period || "") === period);
    return {
      label: periodLabel(period),
      count: events.length,
      players: new Set(events.map((e) => e.playerId).filter(Boolean)).size,
  ...
  const teamRows = project.teams
    .map((team) => {
      const events = filteredEvents.filter(
        (event) => teamForEvent(project, event)?.id === team.id,
      );
      return {
        ...team,
        count: events.length,
        playersTagged: new Set(
          events.map((event) => event.playerId).filter(Boolean),
        ).size,
      };
    })
  ...
  const playerRows = project.teams
    .flatMap((team) =>
      team.players.map((player) => ({
        ...player,
        team,
        count: filteredEvents.filter((event) => event.playerId === player.id)
          .length,
      })),
    )
    ...
    >
      ...
      <div className="bi-report-canvas">
        <header className="bi-report-header">
          <div>
            ...
            <p>
              ...
              {filteredEvents.length} acciones filtradas
            </p>
          </div>
          <div className="bi-header-actions">
            <select
              aria-label="Periodo estadístico"
              value={period}
            ...
            <div className="bi-page-tabs">
              ...
              <button
                className={page === "shooting" ? "active" : ""}
                onClick={() => setPage("shooting")}
              >
                Tiro y zonas
              </button>
              <button
        ...
        <div className="bi-kpi-grid">
          <article className="bi-kpi primary">
            <span>Acciones</span>
            <strong>{filteredEvents.length}</strong>
            <small>{stats.length} tipos distintos</small>
          </article>
          <article className="bi-kpi">
            <span>Jugadores activos</span>
            <strong>{activePlayers}</strong>
            <small>con registro en el filtro</small>
          </article>
          <article className="bi-kpi">
            <span>Tiempo etiquetado</span>
            <strong>{formatTime(taggedDuration)}</strong>
            <small>{intervalCount} intervalos</small>
          </article>
          <article className="bi-kpi">
            <span>Acierto registrado</span>
            <strong>
              {shotPercentage === "—" ? "—" : `${shotPercentage}%`}
            </strong>
            <small>
              {madeShots} de {shots} tiros de campo
            </small>
          </article>
        </div>
        ...
        {page === "overview" ? (
          <>
            <div className="advanced-metrics">
              {[
                ["Puntos", box.points, "Anotación etiquetada"],
                [
                  "eFG%",
                  box.efg === null ? "—" : box.efg.toFixed(1) + "%",
                  "(TC + 0,5 × T3) / intentos",
                ],
                ["Asistencias", box.assist, "Registradas"],
                [
                  "Rebotes",
                  box.rebounds,
                  box.oreb + " ofensivos · " + box.dreb + " defensivos",
                ],
            ...
            <div className="bi-visual-grid">
              {visibleVisuals.includes("trend") && (
                <article className="bi-visual wide">
                  <header>
                    <div>
                      <span>Evolución</span>
                      <h2>Actividad durante el partido</h2>
                    </div>
                    <em>12 segmentos</em>
                  </header>
              ...
              {visibleVisuals.includes("tags") && (
                <article className="bi-visual">
                  <header>
                    <div>
                      <span>Distribución</span>
                      <h2>Acciones por etiqueta</h2>
                    </div>
                  </header>
                  {stats.length ? (
                    <HorizontalBars rows={stats} />
                  ) : (
                    <div className="empty-inline">Sin datos.</div>
                  )}
                </article>
              )}
              {visibleVisuals.includes("teams") && (
                <article className="bi-visual">
                  <header>
                    <div>
                  ...
                  <div className="bi-team-table">
                    {teamRows.map((team) => (
                      <div
                        key={team.id}
                        style={{ "--row-color": team.primaryColor }}
                      >
                        {team.logo ? (
                        ...
                        <div>
                          ...
                          <small>{team.playersTagged} jugadores</small>
                        </div>
                        <b>{team.count}</b>
                      </div>
                    ))}
              ...
              {visibleVisuals.includes("players") && (
                <article className="bi-visual wide player-ranking-visual">
                  <header>
                    <div>
                      <span>Detalle</span>
                      <h2>Participación por jugador</h2>
                    </div>
                    <em>Top 12</em>
                  </header>
                  <div className="bi-player-ranking">
                    {playerRows.map((player, index) => (
                      <div
                        key={player.id}
                        style={{ "--row-color": player.team.primaryColor }}
                      >
                        <em>{index + 1}</em>
                        <span className="bi-player-avatar">
                          {player.photo ? (
                    ...
                    {playerRows.length === 0 && (
                      <div className="empty-inline">
                        Todavía no hay jugadores etiquetados.
                      </div>
                    )}
                  </div>
                </article>
          ...
          <article className="workspace-card boxscore-card">
            <header>
              <div>
                <span className="eyebrow">Box score</span>
                <h2>Rendimiento individual</h2>
              </div>
              <small>Solo acciones etiquetadas</small>
            </header>
                  ...
                  {project.teams
                    ...
                    .map(({ player, team, box }) => (
                      <tr key={player.id}>
                        ...
                        <td>
                          {box.made2}/{box.made2 + box.missed2}
                        </td>
                        <td>
                          {box.made3}/{box.made3 + box.missed3}
                        </td>
                        <td>
                          {box.made1}/{box.made1 + box.missed1}
                        </td>
                        <td>{box.rebounds}</td>
                        <td>{box.assist}</td>
                        <td>{box.turnover}</td>
            ...
            <article className="bi-visual shot-map-visual">
              <header>
                <div>
                  ...
                  <h2>Volumen y acierto por zona</h2>
                </div>
                <em>
                  {shotRows.reduce((total, row) => total + row.attempts, 0)} /{" "}
                  {shots} tiros localizados
                </em>
              </header>
            ...
            <article className="bi-visual">
              ...
              <div className="zone-performance-table">
                ...
                {shotRows.map((zone) => (
                  <div key={zone.id}>
                    <span>
                      <i className={`points-${zone.points}`} />
                      {zone.name}
                    </span>
                    <strong>
            ...
            <article className="bi-visual">
              <header>
                <div>
                  ...
                  <h2>Completitud del etiquetado</h2>
                </div>
...
export function StatsPanel({ project }) {
  const teamIds = [project.match?.homeTeamId, project.match?.awayTeamId].filter(
    Boolean,
  );
  const matchTeams = teamIds
    ...
    .filter(Boolean);
  const hasMatch =
    matchTeams.length === 2 && matchTeams[0].id !== matchTeams[1].id;
  if (!hasMatch || project.events.length === 0) {
    return (
      <section className="stats-empty-state">
        ...
        <div>
          <span>
            <b>1</b> Selecciona el vídeo
          </span>
          <span>
            <b>2</b> Vincula el partido
          </span>
          <span>
            <b>3</b> Empieza a etiquetar
          </span>
        </div>
      </section>
    );
