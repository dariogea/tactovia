...
function isMadeShot(event) {
  return event.tagId === "tag-shot-made-2" ||
    event.tagId === "tag-shot-made-3" ||
    normalized(event.tagName).includes("canasta");
}

function isMissedShot(event) {
  return event.tagId === "tag-shot-missed-2" ||
    event.tagId === "tag-shot-missed-3" ||
    normalized(event.tagName).includes("tiro fallado");
}

function hasConcept(event, ids, words) {
  return ids.includes(event.tagId) || words.some((word) => normalized(event.tagName).includes(word));
}
...
function teamMetrics(team, events) {
  ...
  const score = made.reduce(
    (total, event) => total + (Number(event.shotPoints) || (normalized(event.tagName).includes("3") ? 3 : 2)),
    0
  );
  const zones = zoneStats(own).filter((zone) => zone.attempts > 0);
  const bestZone = zones
    .slice()
    .sort((left, right) => right.percentage - left.percentage || right.attempts - left.attempts)[0];
  const busiestZone = zones
    .slice()
    .sort((left, right) => right.attempts - left.attempts || right.percentage - left.percentage)[0];
  const players = (team.players || [])
    .map((player) => {
      const playerEvents = own.filter((event) => event.playerId === player.id);
      const playerMade = playerEvents.filter(isMadeShot).length;
      const playerShots = playerEvents.filter((event) => isMadeShot(event) || isMissedShot(event)).length;
      return {
        id: player.id,
        name: player.name,
    ...
    .sort((left, right) => right.actions - left.actions || right.made - left.made);
...
function buildTeamConclusions(metrics) {
  ...
  if (metrics.players[0]) {
    conclusions.push(
      `${metrics.players[0].name} es el jugador con más participación registrada (${metrics.players[0].actions} acciones).`
    );
  }
  if (conclusions.length === 0) {
    conclusions.push(`${metrics.name} tiene ${metrics.actions} acciones etiquetadas, todavía sin volumen suficiente para conclusiones específicas.`);
  }
  return conclusions;
}
...
function buildPlayerConclusion(player) {
  ...
  if (player.shots) parts.push(`${player.made}/${player.shots} en tiros (${player.shootingPercentage}%)`);
...
export function buildAutomaticAnalysis(project) {
  ...
  const teams = matchTeamIds
    ...
    .map((metrics) => ({ ...metrics, conclusions: buildTeamConclusions(metrics) }));
  ...
  const shots = project.events.filter((event) => isMadeShot(event) || isMissedShot(event));
  const withZone = shots.filter((event) => event.shotZoneId).length;
  const players = teams
    .flatMap((team) => team.players.map((player) => ({ ...player, teamId: team.id, teamName: team.name, conclusion: buildPlayerConclusion(player) })))
    .sort((left, right) => right.actions - left.actions || right.made - left.made);
  const dataQuality = {
  ...
  return {
    ...
    ready: Boolean(project.match && total > 0)
  };
}
